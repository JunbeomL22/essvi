/// Fit SSVI surface to CBOE option data (SPX, NDX, etc.).
///
/// Usage: cargo run --bin fit_cboe -- spx 2026-03-07
///
/// Three functions:
///   parse_cboe   – CSV → Vec<SliceData> sorted by expiry
...
/// Parse a CBOE CSV into sorted Vec<SliceData>.
///
...
fn parse_cboe(path: &str) -> Vec<SliceData> {
    ...
    for line in lines {
        ...
        // Minimum IV threshold to filter out placeholder values
        let min_iv = 0.01;
...
///
/// k_penalty: -1.5 to 0.5 step 0.025, lambda = 500.
...
fn main() {
    ...
    if args.len() < 3 {
        eprintln!("Usage: cargo run --bin fit_cboe -- <ticker> <date>");
        eprintln!("  e.g. cargo run --bin fit_cboe -- spx 2026-03-07");
        std::process::exit(1);
    }
    let ticker = &args[1];
    let date = &args[2];
